...
function ThreeSteps() {
  ...
  return (
    <section id="use" className="relative py-24 px-6 md:px-12 bg-[#f8fafc] overflow-hidden">
      {/* Background Glows */}
      ...
      <div className="relative max-w-7xl mx-auto z-10">
        ...
        <div className="text-center mb-20">
          <motion.h2 
            ...
            viewport={{ once: true }}
            className="text-3xl md:text-6xl font-black text-slate-950 mb-6"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          ...
          <div className="hidden md:block absolute top-1/2 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-slate-300 to-transparent -translate-y-12" />
          ...
          {steps.map((step, index) => (
            ...
            >
              ...
              <div className="relative glass border-slate-200 rounded-[2.5rem] p-10 flex flex-col items-center text-center h-full hover:border-brand-primary/30 transition-all duration-300">
                {/* Step Number Badge */}
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center font-black text-brand-primary text-sm shadow-2xl">
                  0{index + 1}
                </div>

                {/* Icon Circle */}
                <div className="w-16 h-16 rounded-3xl glass border-slate-200 flex items-center justify-center mb-8 shadow-inner group-hover:scale-110 transition-transform">
                  {step.icon}
                </div>

                <h3 className="text-xl md:text-2xl font-bold text-slate-950 mb-4">
                  {step.title}
                </h3>
        ...
        >
          ...
          <Link to="/login">
            <motion.button 
              ...
              whileTap={{ scale: 0.95 }}
              className="px-12 py-5 glass border-slate-200 hover:border-brand-primary text-slate-900 font-black rounded-full flex items-center justify-center gap-3 transition-colors group"
            >
