...
const Dashboard = () => {
  ...
  const handleOpenModal = async (row: RowData) => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.error || "Failed to load call details",
      );
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        {[
          ...
          >
            ...
            <div
              className={`p-4 rounded-2xl bg-slate-900 border border-slate-800 text-${stat.color} group-hover:scale-110 transition-transform duration-500`}
            >
              {stat.icon}
            </div>
          </div>
      ...
      <motion.div
        variants={itemVariants}
        className="glass rounded-[2.5rem] overflow-hidden border-slate-800/50 shadow-2xl"
      >
        <div className="p-8 border-b border-slate-800/50 flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-white">
              Reports ({reports?.period_days ?? 7} days)
            </h2>
          </div>
          {reportsLoading ? (
        ...
        <div className="p-8 space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              {
                label: "Total Calls",
                value: reports?.summary.total_calls ?? 0,
              },
              {
                label: "Appointments",
                value: reports?.summary.total_appointments ?? 0,
              },
              {
                label: "Total Minutes",
                value: reports?.summary.total_minutes?.toFixed(2) ?? "0.00",
              },
              {
                label: "Successful",
                value: reports?.summary.successful_calls ?? 0,
              },
              {
                label: "Unanswered",
                value: reports?.summary.unanswered_calls ?? 0,
              },
              {
                label: "Repeat Callers",
                value: reports?.summary.repeat_callers ?? 0,
              },
              {
                label: "New Callers",
                value: reports?.summary.new_callers ?? 0,
              },
            ].map((metric) => (
              <div
                key={metric.label}
                className="glass rounded-2xl p-4 border border-slate-800/60"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="glass rounded-2xl p-5 border border-slate-800/60">
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-[2px] mb-4">
                Calls Over Time
              </p>
            ...
            <div className="glass rounded-2xl p-5 border border-slate-800/60">
              ...
              <div className="space-y-2">
                {Object.entries(reports?.sentiment_breakdown ?? {}).length ? (
                  Object.entries(reports?.sentiment_breakdown ?? {}).map(
                    ([label, value]) => (
                      <div
                        key={label}
                        className="flex items-center justify-between text-sm"
          ...
          <div className="glass rounded-2xl p-5 border border-slate-800/60">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {Object.entries(
                reports?.summary.appointment_status_distribution ?? {},
              ).length ? (
                Object.entries(
                  reports?.summary.appointment_status_distribution ?? {},
                ).map(([label, value]) => (
                  <div
                    key={label}
                    className="bg-slate-900/60 rounded-xl px-4 py-3 flex items-center justify-between"
                  >
            ...
            <thead>
              <tr className="bg-slate-900/50 border-b border-slate-800">
                {[
                  ...
                  <th
                    key={h}
                    className="px-8 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[2px]"
                  >
                    {h}
                  </th>
                ))}
                  ...
                  >
                    <td className="px-8 py-6">
                      <div className="font-bold text-white">
                        #{row.call_id.slice(-6).toUpperCase()}
                      </div>
                    ...
                    <td className="px-8 py-6 text-sm font-semibold text-slate-300 capitalize">
                      {row.duration
                        ? `${Math.floor(row.duration / 60)}m ${Math.floor(row.duration % 60)}s`
                        : "0s"}
                    </td>
            ...
            >
              ...
              <div className="p-8 grid grid-cols-2 md:grid-cols-4 gap-6 bg-slate-900/30">
                {[
                  {
                    label: "Agent",
                    val: selectedRow.agent_phone || "AI Agent",
                  },
                  { label: "Phone", val: selectedRow.caller_phone || "N/A" },
                  { label: "Status", val: selectedRow.status },
                  {
                    label: "Duration",
                    val: selectedRow.duration
                      ? `${Math.floor(selectedRow.duration / 60)}m ${Math.floor(selectedRow.duration % 60)}s`
                      : "0s",
                  },
                ].map((d, i) => (
                  <div key={i}>
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[2px] mb-1">
                      {d.label}
                    </p>
                  ...
                  <div className="space-y-4">
                    {activeTab === "transcription" ? (
                      selectedRow.transcript?.items?.length ? (
                        selectedRow.transcript.items.map(
                          (
                            item: {
                              role: string;
                              content:
                            ...
                            <div
                              key={idx}
                              className={`flex gap-4 ${item.role === "assistant" ? "flex-row" : "flex-row-reverse"}`}
                            >
                              <div
                                className={`p-4 rounded-3xl max-w-[80%] ${item.role === "assistant" ? "glass bg-brand-primary/5 text-slate-200" : "bg-slate-800/80 text-slate-300"}`}
                              >
                      ...
                      <p className="text-slate-300 leading-relaxed font-medium">
                        {selectedRow.summary ||
                          "Summary generated upon call completion."}
                      </p>
                    )}
                  </div>
                )}
